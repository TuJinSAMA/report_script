...
if __name__ == '__main__':
    ...
    for dir_name in os.listdir(report_dir):
        ...
        if os.path.isdir(dir_path):
            ...
            # 检查是否包含所有必需的文件
            has_all_files, missing_files = check_required_files(dir_path)
            
            if has_all_files:
                print(f"  文件夹 {dir_name} 包含所有必需文件，开始处理...")
                exec_script(dir_path)
            else:
                print(f"  文件夹 {dir_name} 缺少文件，跳过处理")
                missing_info = {
                    '文件夹名': dir_path,
                    '缺少的报告': missing_files
                }
                missing_reports.append(missing_info)
    
    # 打印缺少文件的统计信息
    if missing_reports:
        print("\n=== 缺少文件的目录统计 ===")
        for info in missing_reports:
            print(f"文件夹: {info['文件夹名']}")
            print(f"缺少的报告: {', '.join(info['缺少的报告'])}")
            print("-" * 50)
    else:
        print("\n所有目录都包含完整的文件！")
